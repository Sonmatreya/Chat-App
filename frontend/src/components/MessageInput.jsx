...
const MessageInput = () => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      await sendMessage({
        text: text.trim(),
        image: fileType === "image" ? imagePreview : null,
        pdf: fileType === "pdf" ? imagePreview : null,
      });
    ...
    <div className="p-4 w-full flex flex-col gap-2">
          ...
          <div className="relative">
            {fileType === "image" ? (
              ...
              <div className="w-20 h-20 flex items-center justify-center bg-zinc-800 rounded-lg border border-zinc-700">
                <FileText className="text-zinc-400" size={40} />
              </div>
            )}
            <button
              onClick={removeFile}
      ...
      <form onSubmit={handleSendMessage} className="flex items-center gap-2">
        <div className="flex-1 flex gap-2">
          ...
          >
            {fileType === "pdf" ? <FileText size={20} /> : <Image size={20} />}
          </button>
        </div>
        <button
          type="submit"
